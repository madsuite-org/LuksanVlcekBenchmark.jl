...
function LV.wood_core()
    args = EM.ArgTracer()
    c = EM.ExaCore(concrete = Val(true))
    EM.@add_var(c, x, args; start = (LV.wood_start(i) for i = 1:args))
    con = EM.@add_con(c, LV.wood_constraint(x, k) for k in 1:args-7)
    EM.@add_con!(c, con, k => LV.wood_constraint_aug(x, k) for k in 1:args-7)
    EM.@add_obj(c, LV.wood_objective(x, i) for i = 1:args÷2-1)
    return c
end
