...
function LV.Chained_HS49_core()
    args = EM.ArgTracer()
    c = EM.ExaCore(concrete = Val(true))
    nC = 2 * (args - 2) ÷ 3
    It_L1 = [3 * div(i-1, 2) for i in 1:2:nC-1]
    It_L2 = [3 * div(i-1, 2) for i in 2:2:nC]
    EM.@add_var(c, x, args; start = (LV.Chained_HS49_start(i) for i = 1:args))
    EM.@add_con(c, LV.Chained_HS49_constraint1(x, l) for l in It_L1)
    EM.@add_con(c, LV.Chained_HS49_constraint2(x, l) for l in It_L2)
    EM.@add_obj(c, LV.Chained_HS49_objective(x, i) for i in 1:floor(Int, (args-2)/3))
    return c
end
